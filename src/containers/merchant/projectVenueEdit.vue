  ...
  <div>
    ...
    <el-form ref="vernue" :model="vernue" :rules="rules" label-width="200px">
      ...
      <div v-if="[7, 8, 9].indexOf(vernue.proId) > -1">
        ...
        <div class="other-area-wrap" v-for="(item, index) in vernue.other1" :key="index">
          ...
          <el-form-item label="该区域最多容纳人数">
            <el-input class="input-wrap" placeholder="人" type="text" v-model="item.count"></el-input>
          </el-form-item>
          <el-form-item label="设施设备/服务">
      ...
      <div v-if="[24, 42].indexOf(vernue.proId) > -1">
        ...
        <el-form-item label="是否接待无监护人陪同未成年人">
      ...
      <div v-if="[20, 21, 22, 23].indexOf(vernue.proId) > -1">
        ...
        <el-form-item label="会场数量" prop="venueNum">
          <el-input class="input-wrap" type="text" placeholder="间" v-model="vernue.venueNum"></el-input>
        </el-form-item>
        <el-form-item label="最大会场面积" prop="maxVenue">
          <el-input class="input-wrap" type="text"  placeholder="m2" v-model="vernue.maxVenue"></el-input>
        </el-form-item>
        <el-form-item label="最大可容纳人数" prop="maxVenueNum">
          <el-input class="input-wrap" type="text"  placeholder="人" v-model="vernue.maxVenueNum"></el-input>
        </el-form-item>
        <el-form-item label="设施设备/服务">
          <el-select v-model="vernue.equipments" multiple placeholder="请选择">
            <el-option-group
  ...
  data () {
    return {
      ...
      rules: {
        ...
        // branchName: [{ required: true, message: '请输入门店名称' }],
    ...
    getDetail () {
      projectVenueDetail({ id: this.id }).then((result) => {
        ...
        let _vernue = {
          ...
          ...result.data,
          ...result.data.otherModular,
          ...result.data.basic,
          // time: result.data.basic.startTime ? [new Date('2019-08-01 ' + result.data.basic.startTime), new Date('2019-08-01 ' + result.data.basic.endTime)] : [],
          days: result.data.basic.days ? result.data.basic.days.split(':').map(item => Number(item)) : [],
          amountImg: result.data.basic.amountImg && JSON.parse(result.data.basic.amountImg),
          other: result.data.basic.other ? JSON.parse(result.data.basic.other) : [{}],
          equipments: result.data.otherModular.equipments ? result.data.otherModular.equipments.split(':').map(item => Number(item)) : [],
          areaId1: result.data.basic.areaId,
          extend2: result.data.basic.extend2 ? JSON.parse(result.data.basic.extend2) : {},
          img: result.data.basic.img ? JSON.parse(result.data.basic.img) : [],
          other1: (result.data.otherModular && result.data.otherModular.other1) ? JSON.parse(result.data.otherModular.other1) : [{}],
          thumbnail: result.data.otherModular.thumbnail ? JSON.parse(result.data.otherModular.thumbnail) : ''
        }
        this.vernue = _vernue
        if (this.provinceId) {
    ...
    onSubmit () {
      this.$refs['vernue'].validate((valid) => {
        if (valid) {
          ...
          projectEdit(_vernue).then((result) => {
